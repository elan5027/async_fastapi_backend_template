...
async def login(
    ...
    response.set_cookie(
        key="auth",
        domain=".kpopmentor.com",
        value=access_token,
        httponly=True,  # JavaScript에서 접근 불가
        secure=True,  # HTTPS에서만 전송
        samesite="None",  # 외부 사이트에서도 쿠키 전송 가능)
    )
...
async def delete_cookie(response: Response) -> JSONResponse:
    response.delete_cookie(key="auth", path="/", domain=".kpopmentor.com")
    json_response = JSONResponse(content={"message": "Login Success"})
